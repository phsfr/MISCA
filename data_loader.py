...
class TextLoader(Dataset):
    ...
    @classmethod
    def my_read_file(cls, input_dir):
        texts, slots, intents = [], [], []
        data_texts = open(os.path.join(input_dir, 'seq.in'), 'r', encoding='utf-8').readlines()
        data_tags = open(os.path.join(input_dir, 'seq.out'), 'r', encoding='utf-8').readlines()
        data_intents = open(os.path.join(input_dir, 'label'), 'r', encoding='utf-8').readlines()
        for text, tag, intent in zip(data_texts, data_tags, data_intents):
            if len(text.strip()) == 0: continue
            if len(tag.strip()) == 0: continue
            if len(intent.strip()) == 0: continue
            texts.append(text.strip().strip('\n').split('\t')[0].split())
            slots.append(tag.strip().strip('\n').split())
    ...
    def build(self, mode):
        ...
        if self.args.task.startswith('mix'):
            texts, slots, intents = self.read_file(data_path)
        else:
            texts, slots, intents = self.my_read_file(os.path.join(self.args.data_dir, self.args.task, mode))
